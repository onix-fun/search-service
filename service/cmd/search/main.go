...
import (
	...
	"github.com/onix-fun/search/service/internal/platform/runtime"
)
...
func run(logger *slog.Logger, args []string) error {
	if len(args) == 0 {
		return errors.New("usage: search <serve|migrate-index> [flags]")
	}
	...
	case "serve":
		...
		roleValue := flags.String("role", string(runtime.RoleAll), "runtime role: api, indexer or all")
		...
		role, err := runtime.ParseRole(*roleValue)
		if err != nil {
			return err
		}
		return runtime.Run(ctx, cfg, role, logger)
	case "migrate-index":
		...
		if err := runtime.Migrate(ctx, cfg); err != nil {
			return err
		}
